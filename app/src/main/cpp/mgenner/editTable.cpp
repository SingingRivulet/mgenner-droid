...
int editTable::selectByArea(int selectBoxX,int selectBoxXend,int selectBoxY,int selectBoxYend){
    ...
    return find(pb,pe,[](note * n , void * arg){//调用HBB搜索
        auto self = (editTable*)arg;
        if(!n->selected){//未选择就加上选择
            self->selected.insert(n);
            n->selected=true;
...
void editTable::addDisplaied(){
    if(displayBuffer.showing){
        ...
        ::__android_log_print(ANDROID_LOG_INFO, "mgenner","add %f %f\n",displayBuffer.begin , displayBuffer.tone);
        std::unique_ptr<history> hisptr (new history);//插入历史记录
        hisptr->method = history::H_NOTE_ADD;
        auto n = addNote(displayBuffer.begin , displayBuffer.tone , defaultDelay , defaultVolume , defaultInfo);
        n->selected = true;
        selected.insert(n);
